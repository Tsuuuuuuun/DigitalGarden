...
AWS Application Discovery Service(ADS)は、オンプレミスのデータセンターにあるサーバーやデータベースの情報を収集して、AWSへの移行を支援するサービスである。

- ホスト名、IPアドレス、MACアドレスといったサーバーの詳細情報の収集
- CPUやメモリの使用状況の収集
- データベースのバージョンやスキーマ情報の収集
- サーバーやデータベース間のネットワーク接続の依存関係のマッピング
